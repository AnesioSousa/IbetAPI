...
Apostador.findById = (idApostador, result) => {
  sql.query("SELECT * FROM pessoa INNER JOIN apostador ON (pessoa.idPessoa = apostador.idPessoa) WHERE idApostador = ? ", idApostador, (err, res) => {
      if (err) {
          console.log("error: ", err);
          result(err, null);
          return;
...
Apostador.getAll = (result) => {
  sql.query("SELECT * FROM pessoa INNER JOIN apostador ON (pessoa.idPessoa = apostador.idPessoa)", (err, res) =>{
      if (err) {
          console.log("error: ", err);
          result(null, err);
          return;
...
Apostador.updateById = (id, pessoa, result) => {
  sql.query("SET SQL_SAFE_UPDATES=0; UPDATE pessoa INNER JOIN apostador ON pessoa.idPessoa = apostador.idPessoa SET pessoa.nome = ?, SET pessoa.telefone = ?, SET pessoa.dataNascimento = ?, SET pessoa.cpf = ?, WHERE apostador.idApostador = ?; SET SQL_SAFE_UPDATES=1;",
